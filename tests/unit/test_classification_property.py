...
class TestClassificationOutputWellFormedness:
    ...
    @given(
        error=extracted_error_strategy,
        invalid_severity=st.text(
            alphabet=st.characters(whitelist_categories=("L",)),
            min_size=2,
            max_size=15,
        ).filter(lambda s: s.lower() not in VALID_SEVERITIES),
        confidence=confidence_strategy,
        rationale=rationale_strategy,
    )
    @settings(max_examples=100)
    def test_well_formed_output_for_invalid_severity_from_llm(
        self,
        error: ExtractedError,
        ...
        """When the LLM returns an invalid severity value (not in the
        allowed set), the ClassificationResult must still be well-formed
        with severity in {low, medium, high, critical}, confidence in
        [0.0, 1.0], and non-empty rationale.

        Args:
            error: Random ExtractedError generated by Hypothesis.
            invalid_severity: Random string that is NOT a valid severity.
            confidence: Random confidence score in [0.0, 1.0].
            rationale: Random non-empty rationale string.
        """
        mock_response: str = _build_mock_llm_response(
        ...
        # Even with an invalid severity from the LLM, the output must
        # be well-formed with a valid severity.
        assert result.severity in VALID_SEVERITIES, (
            f"severity must be one of {VALID_SEVERITIES} even when LLM "
            f"returned invalid severity '{invalid_severity}', "
    ...
    @settings(max_examples=100)
    def test_well_formed_output_for_out_of_range_confidence_from_llm(
        self,
        error: ExtractedError,
        severity: str,
        rationale: str,
        ...
        """When the LLM returns a confidence score outside [0.0, 1.0],
        the ClassificationResult must still have confidence clamped to
        [0.0, 1.0] and all other fields well-formed.

        Args:
            error: Random ExtractedError generated by Hypothesis.
            severity: Random valid severity level.
            rationale: Random non-empty rationale string.
            raw_confidence: Random confidence outside the valid range.
        """
        # Build candidates with well-separated scores to avoid ambiguity
        # triggering escalation for reasons unrelated to this test.
        all_candidates: List[Dict[str, Any]] = [
            {
                "severity": severity,
            ...
            {
                "severity": "low" if severity != "low" else "medium",
                "confidence_score": raw_confidence - 0.5,
                "rationale": "Alternative candidate.",
            },
        ]
        ...
        # The confidence must be clamped to [0.0, 1.0] regardless of
        # what the LLM returned.
        assert result.severity in VALID_SEVERITIES, (
            f"severity must be one of {VALID_SEVERITIES}, got: {result.severity!r}"
        )
        assert 0.0 <= result.confidence_score <= 1.0, (
            f"confidence_score must be in [0.0, 1.0] even when LLM "
            f"returned {raw_confidence}, got: {result.confidence_score}"
        )
        assert result.rationale and result.rationale.strip(), (
            f"rationale must be non-empty, got: {result.rationale!r}"
        )
    ...
    @settings(max_examples=100)
    def test_well_formed_output_for_empty_rationale_from_llm(
        self,
        error: ExtractedError,
        severity: str,
        confidence: float,
    ) -> None:
        """When the LLM returns an empty or whitespace-only rationale,
        the ClassificationResult must still have a non-empty rationale
        (the agent should provide a fallback).
        ...
        # Even with an empty rationale from the LLM, the output must
        # have a non-empty rationale (fallback text).
        assert result.severity in VALID_SEVERITIES, (
            f"severity must be one of {VALID_SEVERITIES}, got: {result.severity!r}"
        )
        assert 0.0 <= result.confidence_score <= 1.0, (
        ...
        assert result.rationale and result.rationale.strip(), (
            "rationale must be non-empty even when LLM returned empty rationale"
        )
